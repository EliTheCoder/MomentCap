...
public:
	CRegister(IConsole *pConsole, IEngine *pEngine, IHttp *pHttp, int ServerPort, unsigned SixupSecurityToken);
...
void CRegister::CProtocol::SendDeleteIfRegistered(bool Shutdown)
{
	...
	std::shared_ptr<CHttpRequest> pDelete = HttpPost(g_Config.m_SvRegisterUrl, (const unsigned char *)"", 0);
...
CRegister::CRegister(IConsole *pConsole, IEngine *pEngine, IHttp *pHttp, int ServerPort, unsigned SixupSecurityToken) :
	m_pConsole(pConsole),
	m_pEngine(pEngine),
	m_pHttp(pHttp),
	m_ServerPort(ServerPort),
...
IRegister *CreateRegister(IConsole *pConsole, IEngine *pEngine, IHttp *pHttp, int ServerPort, unsigned SixupSecurityToken)
{
	return new CRegister(pConsole, pEngine, pHttp, ServerPort, SixupSecurityToken);
}
